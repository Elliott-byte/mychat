...
- **手机上同样好用** — 响应式布局,📷 按钮直接调起摄像头,输入框始终浮在软键盘之上,还能添加到主屏幕当 App 用
...
- 标题自动取自你的第一句话;鼠标悬停可**重命名或删除** —— 重命名直接在原位编辑,删除需再点一次确认,不再弹浏览器原生对话框
...
- **明暗两套主题** —— 页头的太阳/月亮按钮切换;首次访问跟随系统设置,之后记住你的选择
...
- **拍照直接问** — 输入框旁的 **📷** 直接调起后置摄像头,**📎** 从相册选。两种方式都会先在浏览器里
  压缩再上传,5 MB 的照片通常只剩两三百 KB
- 宽度小于 820px 时侧边栏自动收起,点 **☰** 滑出,点旁边空白处收回
- 模型下拉框占满整行;搜索框和 "Free only" 收进 **🔍** 里,需要时再展开
- 复制、重新生成、重命名、删除按钮常驻显示 —— 触屏没有 hover,藏起来就等于没有
- 输入框始终浮在软键盘之上;键盘弹出时,你正在看的那条回复不会被顶走
- 表单文字统一 16px,iOS 点输入框时不会再把整个页面放大
- 适配刘海屏和底部横条;下拉过头也不会触发下拉刷新把页面刷掉
- 用浏览器的**添加到主屏幕**,就能全屏运行并带自己的图标
- 历史存在云端,手机和电脑看到的是同一份
